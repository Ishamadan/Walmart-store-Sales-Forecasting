...
class Train:
    ...
    def train_predict(self,sales_train,features_train,sales_test,features_test):
        ...
        #ac = regressor.score(X_test,Y_test)
        #print(ac)
...
'''    y_ = [[],[]]
    ...
    for d in dept:
        train_d = training_data[training_data.Dept == d]
        test_d = testing_data[testing_data.Dept == d]
         
        if len(train_d) == 0 or len(test_d)==0:
            pass
        else:
            X_train = np.array(train_d.drop(['Weekly_Sales'], axis=1))
            Y_train = np.array(train_d['Weekly_Sales'])

            X_test = np.array(test_d.drop(['Weekly_Sales'], axis=1))
            Y_test = np.array(test_d['Weekly_Sales'])
...
'''y,y_p =train(sales_train,sales_test,1)

# y_pred and y_test
import matplotlib.pyplot as plt
plt.figure(figsize=(20,5))
plt.ylabel('Weekly sale')
plt.plot(y_p[:1000],label='weekly Sales predicted value')
plt.plot(y[:1000],label='weekly sales actual value')
plt.legend(loc="best",shadow=True,fontsize="x-small")

#plt.axis([0,40,0,40])
plt.show()   # along with lin'''
